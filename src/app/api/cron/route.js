...
export async function POST(request) {
  ...
  try {
    ...
    log.push(`Found ${urls.length} URLs with ${urls.reduce((s, u) => s + u.keywords.length, 0)} active keywords`);
    ...
    for (const url of urls) {
      ...
      try {
        ...
        log.push(`DFS: ${url.title} — ${Object.keys(dfsData).length} keyword results`);
      } catch (e) {
      ...
      for (const kw of url.keywords) {
        ...
        if (prevPos && currentPos) {
          ...
          if (prevPos <= 10 && currentPos > 10) {
            ...
            await prisma.alert.create({ data: { keywordId: alert.keywordId, type: alert.type, severity: alert.severity, details: alert.details } });
      ...
      if (autoAddGsc) {
        try {
          const existingKws = new Set(url.keywords.map(k => k.keyword.toLowerCase()));
          ...
          if (added > 0) log.push(`Auto-added ${added} keywords for ${url.title}`);
        } catch (e) {
      ...
      if (url.keywords.length > 0) {
        ...
        if (droppingKws.length >= url.keywords.length * 0.5) {
          await prisma.trackedUrl.update({ where: { id: url.id }, data: { status: 'declining' } });
        }
      }
    }

    // 4. Send Telegram notification
    const totalAlerts = newAlerts.critical.length + newAlerts.warning.length + newAlerts.positive.length;
    if (totalAlerts > 0) {
      const dashboardUrl = process.env.DASHBOARD_URL || '';
      const message = formatWeeklyReport({
        weekDate: weekStarting.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
